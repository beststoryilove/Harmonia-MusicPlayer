...
if (!count) return { r: 30, g: 30, b: 40 };
return {
...
.dlp-bg{flex-shrink:0;padding:2px 14px 0;font-size:12px;font-weight:600;opacity:0;max-height:0;box-sizing:border-box;text-align:center;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 1px 6px rgba(0,0,0,.4);border-bottom:1px solid rgba(255,255,255,.08);pointer-events:none;transition:opacity .35s ease,max-height .35s ease,padding .35s ease}
.dlp-bg.is-on{opacity:.85;max-height:40px;padding:2px 14px 1px;pointer-events:auto}
.dlp-bg-words{display:inline-block;white-space:nowrap;font-size:12px;font-weight:600;text-shadow:0 1px 6px rgba(0,0,0,.4)}
.dlp-bg-trans{display:block;font-size:9px;font-weight:400;opacity:.55;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.dlp-bg2{flex-shrink:0;padding:1px 14px 2px;font-size:11px;font-weight:600;opacity:0;max-height:0;box-sizing:border-box;text-align:center;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:0 1px 6px rgba(0,0,0,.4);pointer-events:none;transition:opacity .35s ease,max-height .35s ease,padding .35s ease}
.dlp-bg2.is-on{opacity:.7;max-height:40px;padding:1px 14px 1px;pointer-events:auto}
.dlp-bg2-words{display:inline-block;white-space:nowrap;font-size:11px;font-weight:600;text-shadow:0 1px 6px rgba(0,0,0,.4)}
.dlp-bg2-trans{display:block;font-size:9px;font-weight:400;opacity:.5;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
			        ...
			        .dlp-current-inner{display:inline-block;white-space:nowrap;transition:transform .35s cubic-bezier(.25,.8,.25,1);font-size:20px;font-weight:700;text-shadow:0 2px 12px rgba(0,0,0,.4);will-change:transform}
				        .dlp-word{position:relative;display:inline-block;white-space:pre;color:rgba(255,255,255,0.35);transform:translateZ(0);will-change:transform}
				        .dlp-word::after{content:attr(data-t);position:absolute;left:0;top:0;width:var(--p,0%);overflow:hidden;white-space:pre;color:#fff;pointer-events:none}
			        ...
			        .dlp-play-wrap{position:relative;width:36px;height:36px;display:flex;align-items:center;justify-content:center;flex-shrink:0}
				        .dlp-play-btn{position:relative;z-index:2;width:30px;height:30px;border-radius:50%;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.08);color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:12px}
				        .dlp-progress-ring{position:absolute;inset:0;width:100%;height:100%;pointer-events:none;transform:rotate(-90deg)}
				        .dlp-progress-track{fill:none;stroke:rgba(255,255,255,0.18);stroke-width:2}
				        .dlp-progress-fill{fill:none;stroke:rgba(255,255,255,0.85);stroke-width:2;stroke-dasharray:100.53;stroke-dashoffset:100.53;stroke-linecap:round;transition:stroke-dashoffset 0.25s linear}
/* ===== 歌词增强效果 ===== */
.lyric-line.active{
...
transition:opacity 0.3s ease,visibility 0.3s ease;
        ...
        function _updateWordProgress(words, currentTime) {
          ...
          if(_totalWidth>_wrapW&&_wrapW>0){
            ...
            if(activeIdx<0)activeIdx=(currentTime<words[0].s)?0:lastIdx;
        ...
        window.updateLyrics=function(arg){
          ...
          _bgBaseTime = _ct;
          setTimeout(function(){layoutLines();},10);
...
function syncDesktopLyricsPip() {
        ...
        if (amLyricsData && amLyricsData.length) {
          ...
          if (fgPool.length > 0) {
            fg = fgPool[0];
            for (let k=1;k<fgPool.length;k++) bgPure.push(fgPool[k]);
...
document.getElementById('createPlaylistBtn').addEventListener('click', promptCreatePlaylist);
const emptyCreateBtn = document.getElementById('emptyCreatePlaylistBtn');
if (emptyCreateBtn) emptyCreateBtn.addEventListener('click', promptCreatePlaylist);
...
const cacheKey = `v3::${forceSearch ? 'force' : 'auto'}::${queries.join(' | ')}`;
...
const enabled = localStorage.getItem(IDLE_HIDE_KEY) !== 'false';
...
if (existing && existing.source !== 'kugou') continue;
let songs = [];
try { songs = await fetchKugouPlaylistTracks(gid); }
catch (err) { console.error('[kugou playlist tracks]', gid, err); continue; }
