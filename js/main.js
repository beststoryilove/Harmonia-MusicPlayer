...
if (dynamicIslandWelcome) dynamicIslandWelcome.textContent = `欢迎使用Harmonia！ 当前正在播放：${currentSong}`;
}
...
if(typeof currentLyricOffset!=='undefined'&&currentLyricOffset!==0)currentTime+=currentLyricOffset/1000;
...
function setPlaybackRate(rate){currentPlaybackRate=rate;try{audioPlayer.playbackRate=rate;}catch(e){}document.querySelectorAll('.pm-speed-btn').forEach(b=>{b.classList.toggle('active',parseFloat(b.dataset.speed)===rate);});localStorage.setItem('playbackRate',String(rate));}
let playerMoreOverlayEl=null;
function openPlayerMoreModal(){if(!playerMoreOverlayEl)return;playerMoreOverlayEl.classList.add('active');document.body.classList.add('settings-modal-open');}
function closePlayerMoreModal(){if(!playerMoreOverlayEl)return;playerMoreOverlayEl.classList.remove('active');document.body.classList.remove('settings-modal-open');}
const _pmClose=()=>closePlayerMoreModal();
playerMoreOverlayEl=document.getElementById('playerMoreOverlay');
const _pmBtn=document.getElementById('playerMoreBtn');
if(_pmBtn){_pmBtn.addEventListener('click',e=>{e.stopPropagation();openPlayerMoreModal();});_pmBtn.addEventListener('touchend',e=>{if(e.cancelable)e.preventDefault();e.stopPropagation();openPlayerMoreModal();});}
const _pmCloseBtn=document.getElementById('playerMoreClose');
if(_pmCloseBtn)_pmCloseBtn.addEventListener('click',_pmClose);
if(playerMoreOverlayEl)playerMoreOverlayEl.addEventListener('click',e=>{if(e.target===playerMoreOverlayEl)_pmClose();});
...
document.querySelectorAll('.pm-speed-btn').forEach(btn=>{btn.addEventListener('click',()=>{setPlaybackRate(parseFloat(btn.dataset.speed));});});
...
/* 页面加载时启动预览 */
setTimeout(updateTimeDisplayPreview, 500);
...
{ key: 'Escape', desc: '关闭弹窗', action() { if (posterModalOverlay?.classList.contains('active')) closePosterModal(); if (settingsModalOverlay?.classList.contains('active')) { closeSettingsModal(); } if (mvModalOverlay?.classList.contains('active')) closeMvModal(); if (lyricsRerequestModalOverlay?.classList.contains('active')) closeLyricsRerequestDialog(); var _pmo=document.getElementById('playerMoreOverlay'); if(_pmo&&_pmo.classList.contains('active')){closePlayerMoreModal();return;} if (isDynamicIslandExpanded) toggleDynamicIsland(); } },
