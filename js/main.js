...
.dlp-bg2{flex-shrink:0;padding:1px 14px 0;font-size:11px;font-weight:600;opacity:0;max-height:0;box-sizing:border-box;text-align:center;color:${fg};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:none;border-bottom:1px solid ${fgAlpha}0.12);pointer-events:none;transition:opacity .35s ease,max-height .35s ease,padding .35s ease}
...
const covers = pl.tracks.slice(0, 4).map(t => t.cover || t.pic_id).filter(Boolean);
pl.cover = covers;
}
...
function loadTranslationSettings() {
	...
	if (savedTrans) {
		...
		let rawToken = translationSettings.apiToken || '';
		try { rawToken = atob(rawToken); } catch {}
		apiTokenInput.value = rawToken;
...
function saveTranslationSettings() {
	...
	let encodedToken = plainToken;
	try { encodedToken = btoa(plainToken); } catch {}
	...
	'translationSettings',
	JSON.stringify({ ...translationSettings, apiToken: encodedToken })
	);
}
...
// （快速交替对唱无需与 fg 重叠）；对唱优先、组内开始最早、跳过同文本、最多 maxBg 条
...
// 2) 副行：仅对唱/背景行，当前活跃（inWindow）即展示；对唱优先、组内开始最早、跳过同文本、最多 maxBg 条
let bgSlots = [];
if (fg) {
const fgText = txt(fg);
...
async function translateLyrics(lyricText) {
	...
	const prompt = `请将以下歌词翻译为中文（简体）。这是一首名为《${songName}》的${artist ? '由 ' + artist + ' 演唱' : ''}的歌曲。

要求：
- 使用以下格式，每行一条：原文 => 译文（不要加方括号）
- 保留时间轴标记，格式如 [00:01.23] => [翻译]
- 翻译要富有文采，符合中文歌词的韵律
- 不要添加任何额外的说明或注释
- 已经是中文的部分（如中文歌词、中文标注）保持原样，不要翻译，直接输出原文即可
...
const raw = `${songName}||${artist}||${model}`;
let hash = 0;
for (let i = 0; i < raw.length; i++) {
hash = ((hash << 5) - hash + raw.charCodeAt(i)) | 0;
}
return TRANS_CACHE_PREFIX + Math.abs(hash).toString(36);
}
...
/* 标签标题映射 */
const TAB_TITLES={timer:'睡眠定时',speed:'倍速播放',playlist:'歌单操作'};
...
const attemptedToday = localStorage.getItem(KUGOU_VIP_DAILY_ATTEMPTED_KEY) === todayKey;
let res;
if (!attemptedToday) {
try {
localStorage.setItem(KUGOU_VIP_DAILY_ATTEMPTED_KEY, todayKey);
res = await wrappedFetch(buildKugouApiUrl('/youth/vip', { receive_day: todayKey }, true, false), { credentials: 'include' });
} catch (newApiErr) {
console.warn('[VIP] 新接口失败，降级旧接口:', newApiErr.message);
res = null;
}
if (!res || !res.ok) {
try {
...
throw new Error('领取 VIP 请求失败（新/旧接口均不可用）');
}
}
} else {
res = await wrappedFetch(buildKugouApiUrl('/youth/vip', {}, true, false), { credentials: 'include' });
...
/* ===== 歌词胶囊（顶部液态玻璃） ===== */
.pip-lyrics-pill{position:absolute;top:12px;left:50%;transform:translateX(-50%);z-index:5;display:flex;flex-direction:column;align-items:center;gap:2px;max-width:calc(100% - 96px);padding:7px 16px;border-radius:20px;background:var(--pip-glass-bg,rgba(30,30,40,0.35));backdrop-filter:blur(42px) saturate(140%) contrast(105%);-webkit-backdrop-filter:blur(42px) saturate(140%) contrast(105%);border:1px solid var(--pip-glass-border,rgba(255,255,255,0.22));box-shadow:0 12px 32px rgba(0,0,0,0.45),inset 0 1.5px 0 rgba(255,255,255,0.38),inset 0 -1px 0 rgba(0,0,0,0.25);transition:opacity .35s ease,transform .35s ease;pointer-events:none;color:var(--pip-fg,#fff);}
...
.pip-lyric-main{font-size:13px;font-weight:700;white-space:nowrap;max-width:100%;overflow:hidden;}
...
.pip-lyric-append-body{overflow:hidden;min-height:0;}
...
pipRenderLine(appEl,_pillAppendWords,_pillBaseTime,true);
// 追加行翻译:写入 + 缓存行时间轴(与主行翻译滚动同款)
var _pat=pipEl('pipLyricAppendTrans');
var _ant=append.translation||'';
if(_pat.textContent!==_ant){
_pat.textContent=_ant;
(updatePipLyrics)._appTransMax=null;
...
(updatePipLyrics)._appTransTime=append.time||0;
(updatePipLyrics)._appTransEnd=append.words&&append.words.length?(append.words[append.words.length-1].e||0):0;
if((updatePipLyrics)._appTransEnd<=(updatePipLyrics)._appTransTime)(updatePipLyrics)._appTransEnd=(updatePipLyrics)._appTransTime+5;
...
var songTime=_pillBaseTime+elapsed;if(songTime<0)songTime=0;
	...
	if(_pillAppendWords.length)pipRenderLine(pipEl('pipLyricAppendInner'),_pillAppendWords,songTime,true);
		...
		// 追加行翻译过长时进度滚动(同款居中逻辑)
		var _pat3=pipEl('pipLyricAppendTrans');
		if(_pat3&&_pat3.textContent){
		var _patW3=_pat3.scrollWidth||0;
		var _patBox3=_pat3.getBoundingClientRect().width;
		var _patWrap0=(updatePipLyrics)._appTransWrap||0;
		if(Math.abs(_patBox3-_patWrap0)>0.5){(updatePipLyrics)._appTransWrap=_patBox3;(updatePipLyrics)._appTransMax=null;}
		var _patMax3=Math.max(0,_patW3-_patBox3);
		if(_patMax3>0){
		if((updatePipLyrics)._appTransMax==null){(updatePipLyrics)._appTransMax=_patMax3;}
		var _att3=(updatePipLyrics)._appTransTime||0,_ate3=(updatePipLyrics)._appTransEnd||(_att3+5);
		var _apr3=Math.max(0,Math.min(1,(songTime-_att3)/Math.max(0.001,_ate3-_att3)));
		var _apos3=_apr3*_patW3-_patBox3/2;
		_apos3=Math.max(0,Math.min(_apos3,_patMax3));
		_pat3.style.transform='translateX('+(-_apos3.toFixed(2))+'px)';
		_pat3.style.textAlign='left';
		} else if(_pat3.style.transform||_pat3.style.textAlign!=="center"){ _pat3.style.transform=''; _pat3.style.textAlign='center'; }
		}
});
})();
